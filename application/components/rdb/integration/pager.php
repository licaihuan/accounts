...
class Pager
{
	...
	{/*{{{*/
		...
		{
			self::$executor = LoaderSvc::loadExecutor();
		}else
		{
	 ...
	 *	实现查询
	 */
	static public function render($options)
	{/*{{{*/
		try
		{
			self::init($options);
	...
	public static function getPageStr($page=1,$pagesize=20,$rows,$baseurl, $varName = 'p')
	{
		...
		if($count<=8)
			{
				for($i=1;$i<=$count;$i++)
				{
					if($i==$page)
					{
						$pageStr.=' <a class="number current" href="javascript:void(0)">'.$i.'</a> ';
						//$pageStr.="<span class='on'>".$i."</span>";
					}
					else
					{
						$pageStr.=' <a class="number" href="'.$baseurl.$varName.'='.$i.'">'.$i.'</a> ';
		...
		elseif($page<=4 && $count> 8)
		{
			...
			for($i=1;$i<=6;$i++)
			{
				if($i==$page)
					...
					{
						$pageStr.=' <a class="ajax-link" href="'.$baseurl.$varName.'='.$i.'">'.$i.'</a> ';
						//$pageStr.="<a target='_self' href='".$baseurl."$varName=".$i."' >".$i."</a>";
					}
			...
			if($page==$count)
			{
				$pageStr.=' <a class="number current" href="javascript:void(0)">'.$count.'</a> ';
				//$pageStr.="...<span class='on'>".$count."</span>";
			}
			else
			{
				$pageStr.=' <a class="number" href="'.$baseurl.$varName.'='.$count.'">'.$count.'</a>';
		...
		elseif($page>4 && $page <= $count-3 && $count>= 8)
		{
			...
			$pageStr.='<a class="number" href="'.$baseurl.$varName.'=1">1</a>';
		...
		if($nextpage==-1)
		{
			$pageStr.= '<a class="" href="javascript:void(0)">下一页 → </a>';
			//$pageStr.="<a>下一页</a>";
		}
		else
		{
			$pageStr.= '<a class="" href="'.$baseurl."$varName=".$nextpage.'">下一页 → </a>';
			//$pageStr.="<a target='_self' href='".$baseurl."$varName=".$nextpage."' >下一页</a>";
		}
        if(APP_NAME !== 'front'){
            $pageStr.='<form id="gotopage" method="GET" action="'.$baseurl.'p=" >';
            $pageStr.= '<b><i>&nbsp;&nbsp;转至&nbsp;&nbsp;</i>';
            $pageStr.='<input id="gotopage_p" type="text" class="text"  style="width:70px;"/>';
            $pageStr.= '<button class="btn" onclick="javascript:gotopage(\'gotopage\');return false;" style="margin-left:10px;">GO</button>';
            $pageStr.= '</form>';
            $pageStr.= <<<EOF
            <script>
            function gotopage(id){
	            var form = document.getElementById(id);
	            var url =  form.getAttribute('action') + document.getElementById(id + '_p').value;
...
EOF;
	...
	public static function pageStr($page=1,$pagesize=20,$rows,$baseurl, $varName = 'p')
	{
		...
		//$pageStr.= '<input name="" type="button" class="btn" onclick="$(\'#gotopage\').submit();"/>';
		//$pageStr.= '</form>';
	...
	public static function pageStr3($page=1,$pagesize=20,$rows,$baseurl, $varName = 'p')
	{
		...
		$pageStr.= '<input name="" type="button" class="btn" onclick="goPage(document.getElementById(\'p\').value,'.$count.',\''.$baseurl.'\')"/';
		$pageStr.= '</form>';
		return $pageStr;
