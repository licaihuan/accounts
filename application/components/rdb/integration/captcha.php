...
class Captcha
{
	const CAPTCHA_FONT_FILE = 'verdana.ttf';
	const FONT_SIZE = '14';
	...
	public function show_image($width = 88, $height = 31)
	{
		...
		if(!file_exists(self::CAPTCHA_FONT_FILE)) exit('CAPTCHA_FONT_FILE NOT FOUND');

		if($this->_chars_number < 3) exit('CAPTCHA LESS THEN 3');
		...
		for($i = 0; $i < $this->_chars_number; $i++)
		{
			...
			imagettftext($im, self::FONT_SIZE, $angle, $x, $y, $textcolor, $_SERVER['CAPTCHA_FONT_FILE'], $char);
		}

	
		LoaderSvc::loadSess()->set(self::CAPTCHA_SESSIONNAME,$string);
	...
	{
		$save_code = strtolower(LoaderSvc::loadSess()->get(Captcha::CAPTCHA_SESSIONNAME));
		LoaderSvc::loadSess()->destroy(Captcha::CAPTCHA_SESSIONNAME);
